...
\begin{proof}[Proof of Theorem 25.3]
    ...
    that $\left\{X_{t}\right\} \stackrel{\mathrm{d}}{=}\left\{X_{t}^{0}+\right.$ $\left.X_{t}^{1}\right\}$ and
    ...
    Conversely, suppose that $\int g(y) \nu_{1}(\mathrm{~d} y)<\infty$. Let us prove
...
subexponenthal if $\mu(x, \infty)>0$ for every $x$ and
